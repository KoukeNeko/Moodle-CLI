...
<#
.SYNOPSIS
    Install Moodle CLI on Windows.

.DESCRIPTION
    The archive is never trusted on its own: this downloads the release's
    checksums.txt alongside it and refuses to install anything whose digest
    does not match, so an interrupted or tampered download fails loudly
    instead of leaving a broken binary on PATH.

    Windows binaries are not Authenticode-signed, so SmartScreen may warn even
    though the checksum matches the release.

.PARAMETER Version
    Release tag to install, for example v0.1.4. Defaults to the latest stable
    release, which never resolves to a pre-release.

.PARAMETER InstallDir
    Directory to install into. Defaults to %LOCALAPPDATA%\Programs\moodle-cli.

.EXAMPLE
    irm https://raw.githubusercontent.com/KoukeNeko/Moodle-CLI/main/scripts/install.ps1 | iex

.EXAMPLE
    .\install.ps1 -Version v0.1.4 -InstallDir C:\Tools\moodle
#>
[CmdletBinding()]
param(
    [string]$Version,
...
function Assert-Checksum {
    ...
    # The file lists every archive of the release, so the name has to be
    # matched rather than the first line read.
    $expected = $null
    ...
    if (-not $expected) { throw "$ArchiveName is not listed in checksums.txt." }
...
$archiveName = "moodle-cli_${numericVersion}_windows_${architecture}.zip"

$workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("moodle-cli-install-" + [System.Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Invoke-WebRequest -Uri "$DownloadBase/$resolvedVersion/checksums.txt" -OutFile $checksumPath
    ...
    $extracted = Join-Path $workDir 'moodle.exe'
    if (-not (Test-Path -LiteralPath $extracted)) { throw 'The archive did not contain moodle.exe.' }

    New-Item -ItemType Directory -Path $InstallDir -Force | Out-Null
    $target = Join-Path $InstallDir 'moodle.exe'
    ...
    # Not "completion": that name belongs to Moodle's own activity completion.
    Write-Host 'Shell completion: moodle shell-completion powershell | Out-String | Invoke-Expression'
} finally {
